...
Figure illustrates the stepwise insertion of matches by rules into the final annotation. Custom defined rules filter proper matches that fit certain feature criteria from all matches. These candidates are sorted by decreasing scores (numbers 1-9, 1 is the highest scoring, top match). Next, the pipeline checks whether it can insert a candidate, starting from the highest to the lowest score. In the example, match 4 - despite having a higher score than match 8, is not inserted because it overlaps with match 1. Similarly, match 3 but not match 6 is selected. After all candidates of one round have been evaluated for a potential insertion, the next round with lower stringency continues. The final annotation is obtained after the full sequence of rounds and rules have been completed.
