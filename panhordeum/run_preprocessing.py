...
def runbatch(genotype : str, genomefile : str, projrootdir : str, tfasta: str, pfasta: str, offsetfile: str,
             mmap2 : str):

    # first make output directory
    try:
        os.mkdir(projrootdir)
    except OSError:
        # TO DO!!!: add option force if overwrite possible, currently overwrites
        # existing results in outdir
        pass

    outdir = os.path.join(projrootdir, genotype)
    try:
        os.mkdir(outdir)
    except OSError:
        # TO DO!!!: add option force if overwrite possible, currently overwrites
...
if __name__ == '__main__':
    ...
    argParser.add_argument("-o", "--outroot", type=str,
                           help="output directory, created if not present. Root directory for all project files and directories")
    ...
    for gt, genomefile in runlist[args.batchstart:args.batchend+1]:
        runbatch(genotype=gt, genomefile=genomefile, projrootdir=args.outroot, tfasta=args.tfasta,
